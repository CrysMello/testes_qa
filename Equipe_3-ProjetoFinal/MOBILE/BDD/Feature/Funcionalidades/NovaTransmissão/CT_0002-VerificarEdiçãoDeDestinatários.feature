#language: pt
#encoding: UTF-8
#author:Equipe Trios
#date: 15/11/2021
#version: 1.0

Funcionalidade: <Nova transmissão>
User Story
Eu <Equipe Trios> como usuario do "Whatsapp"
Quero adicionar novos contatos
Para inserir os 256 contatos da "lista transmissão"         

Contexto: 
       Dado que acesso o aplicativo do "Whatsapp"
       E clico em editar destinatários 
       E clico em pesquisar
       E seleciono os 256 constatos

       Então inclusão criada com sucesso

ID: CT_0002
Cenario: Verificar edição de destinatários

      Quando edito os destinatários do "Whatsapp"
      E seleciono os contatos
      E clico em adicionar destinatário
        
      Então destinatários adicionados com sucesso

Esquema do cenario:
Exemplos:
|contatos |
|Crystiane|
|Cleide   |
|Michelle |